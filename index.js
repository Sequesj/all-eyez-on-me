...
client.on("messageCreate", async(message) => {
    ...
    if (message.content.toLowerCase() === "!proc") {
        sql.query(`SELECT UserID, Coins FROM User WHERE UserID = ${message.author.id}`, (err, rows, fields) => {
            if (err) {
                console.log(err);
            } else if (rows && rows.length > 0) {
                let balance = rows[0].Coins;

                if (balance >= 1000) {
                    sql.query(`UPDATE User SET Coins = Coins - 1000 WHERE UserID = ${message.author.id}`);
                    message.reply(`Монеты сняты. Ваш баланс: ${balance - 1000}.`);
                } else
                    message.reply(`Недостаточно монет. Ваш баланс ${balance}`);
            } else
...
client.api.applications("897893301653495828").guilds('790275150816870401').commands.post({
    data: {
        name: 'find',
